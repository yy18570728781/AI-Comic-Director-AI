...
function ResourcesTab({ scriptId }: ResourcesTabProps) {
  ...
  const handleSaveCharacters = async () => {
    ...
    const scenesToSave = extractedScenes.filter((s) =>
      selectedScenes.includes(s.name),
    );
      ...
      >
        <div style={{ maxHeight: '70vh', overflow: 'auto' }}>
          ...
          <div style={{ 
            ...
            <div>
              <h4 style={{ margin: 0, fontSize: '16px', fontWeight: 600 }}>从剧本内容和分镜数据中提取</h4>
              <p style={{ margin: '4px 0 0 0', color: '#666', fontSize: '14px' }}>
            ...
            <Space>
              {!hasExtracted ? (
                ...
                <>
                  ...
                  <Button type="primary" loading={saving} onClick={handleSaveCharacters} disabled={selectedCharacters.length === 0 && selectedScenes.length === 0}>
          ...
          {extracting && (
            <div style={{ textAlign: 'center', padding: '40px' }}>
              <Spin size="large" />
              <div style={{ marginTop: '16px', color: '#666' }}>正在分析剧本内容，提取角色和场景信息...</div>
            </div>
          )}
              ...
              {(extractedCharacters.length > 0 || extractedScenes.length > 0) ? (
                <>
                  ...
                  {extractedCharacters.length > 0 && (
                    <>
                      <h4 style={{ margin: '16px 0 12px 0', color: '#333' }}>角色 ({extractedCharacters.length})</h4>
                        ...
                        {extractedCharacters.map((character) => {
                          ...
                          return (
                            <Card
                              ...
                              cover={
                                <div style={{ width: '100%', height: 120, backgroundColor: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#999' }}>
                              ...
                              <Card.Meta
                                title={
                                  <div style={{ fontSize: 14, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {character.name}
                                    {character.variant && character.variant !== '默认' && (
                                      <Tag color="purple" style={{ marginLeft: 8, fontSize: 11 }}>{character.variant}</Tag>
                                    )}
                                  </div>
                                }
                                description={
                                  <div>
                                    {character.tags && character.tags.length > 0 && (
                                      <div style={{ marginBottom: 6 }}>
                                        {character.tags.map((tag, idx) => (
                                          <Tag key={idx} color="blue" style={{ fontSize: 10, marginBottom: 2 }}>{tag}</Tag>
                                        ))}
                                      </div>
                                    )}
                                    {character.description && (
                                      <div style={{ fontSize: 12, color: '#666', marginBottom: 6, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: '1.3' }}>
                                        {character.description}
                                      </div>
                                    )}
                  ...
                  {extractedScenes.length > 0 && (
                    <>
                      <h4 style={{ margin: '16px 0 12px 0', color: '#333' }}>场景 ({extractedScenes.length})</h4>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
                          ...
                          return (
                            <Card
                              ...
                              style={{
                                borderRadius: '12px',
                                overflow: 'hidden',
                                border: isSelected ? '2px solid #52c41a' : '1px solid rgba(82, 196, 26, 0.2)',
                                cursor: 'pointer',
                                boxShadow: isSelected ? '0 4px 16px rgba(82, 196, 26, 0.4)' : '0 2px 8px rgba(0, 0, 0, 0.1)',
                                transition: 'all 0.3s ease'
                              }}
                              styles={{ body: { padding: '16px' } }}
                              onClick={() => {
                                if (isSelected) {
                                  setSelectedScenes((prev) => prev.filter((name) => name !== scene.name));
                                } else {
                                  setSelectedScenes((prev) => [...prev, scene.name]);
                                }
                              }}
                              cover={
                                <div style={{ width: '100%', height: 120, backgroundColor: '#f6ffed', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#52c41a' }}>
                            ...
                            >
                              <Card.Meta
                                ...
                                description={
                                  <div>
                                    {scene.description && (
                                      <div style={{ fontSize: 12, color: '#666', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: '1.3' }}>
                                        {scene.description}
                                      </div>
                                    )}
                                  </div>
  ...
  const renderSceneContent = () => (
    <Card style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div>
          <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 600, color: '#333' }}>已保存场景</h3>
          <p style={{ margin: '4px 0 0 0', color: '#666', fontSize: '14px' }}>
            当前用户已保存到资源库的所有场景
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {savedScenes.map((scene) => (
            <Card
              ...
              cover={
                <div style={{ width: '100%', height: 200, backgroundColor: '#f6ffed', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                  {scene.images?.[0]?.url ? (
                    <Image
                      ...
                      height={200}
                      style={{ objectFit: 'contain' }}
                      preview={{ mask: <div style={{ color: '#fff', fontSize: 14 }}>{scene.name}</div> }}
                  ...
                  <div style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.6)', borderRadius: 4, padding: '2px 6px' }}>
                    <Tag color="green" style={{ margin: 0, fontSize: 11 }}>{scene.id}</Tag>
                  </div>
                </div>
              }
            >
              <Card.Meta
                ...
                description={
                  <div>
                    {scene.tags?.length > 0 && (
                      <div style={{ marginBottom: 8 }}>
                        {scene.tags.map((tag: string, idx: number) => (
                          <Tag key={idx} color="green" style={{ fontSize: 11, marginBottom: 4 }}>{tag}</Tag>
                        ))}
                      </div>
                    )}
                    {scene.description && (
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <Button size="small" type="primary" icon={<PictureOutlined />} onClick={() => message.info('场景图像生成功能开发中...')}>
                        生成图像
                      </Button>
                      <Button size="small" icon={<UploadOutlined />} onClick={() => message.info('图像上传功能开发中...')}>
                        上传图像
                      </Button>
                      <Button size="small" danger icon={<DeleteOutlined />} onClick={() => message.info('场景删除功能开发中...')}>
                        删除
                      </Button>
                    </div>
                  </div>
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ 
          ...
          {/* 提取按钮移到右侧 */}
          <Button type="primary" icon={<UserAddOutlined />} onClick={() => setExtractModalVisible(true)}>
            提取角色和场景
          </Button>
        </div>
      </div>
